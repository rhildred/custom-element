...
function b(c, n) {
  if (n) {
    if (W.test(c))
      return c.replace(te, U);
  } else if (G.test(c))
    return c.replace(ne, U);
  return c;
}
...
`, n = n.substring(u.length + 1), h = !0), !h) {
          const B = new RegExp(`^ {0,${Math.min(3, m - 1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`), j = new RegExp(`^ {0,${Math.min(3, m - 1)}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`), O = new RegExp(`^ {0,${Math.min(3, m - 1)}}(?:\`\`\`|~~~)`), H = new RegExp(`^ {0,${Math.min(3, m - 1)}}#`);
          for (; n; ) {
            const M = n.split(`
`, 1)[0];
            if (u = M, this.options.pedantic && (u = u.replace(/^ {1,4}(?=( {4})*[^ ])/g, "  ")), O.test(u) || H.test(u) || B.test(u) || j.test(n))
              break;
            if (u.search(/[^ ]/) >= m || !u.trim())
              o += `
` + u.slice(m);
            else {
              if (y || g.search(/[^ ]/) >= 4 || O.test(g) || H.test(g) || j.test(g))
                break;
              o += `
` + u;
            }
